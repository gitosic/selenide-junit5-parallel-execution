...
public class ChromiumHelper {
    public static final String baseUrlYandex = System.getProperty("base.url", "https://ya.ru/");
    // "chrome" or "chrome-headless"
    private static final String browserType = System.getProperty("browser.type", "chrome");
    ...
    public static Path getDriverPath() {
        ...
        if (Platform.getCurrent().is(Platform.WINDOWS)) {
            // You can add "chromedriver.exe" for Windows
            driverPath = Path.of("src\\test\\resources\\chromedriver\\chromedriver.exe");
            if (!driverPath.toFile().exists()) {
                driverPath = Path.of("plcm-bdd-ui-tests\\src\\test\\resources\\chromedriver\\chromedriver.exe");
            }
        } else {
            ...
            if (!driverPath.toFile().exists()) {
                driverPath = Path.of("plcm-bdd-ui-tests/src/test/resources/chromedriver/chromedriver");
            }
        }
        return driverPath;
    }
    ...
    private ChromeDriverService getBrowserService() {
        return BROWSER_SERVICE; //ChromeDriverService.createDefaultService();
    }

    private WebDriver getWebDriver(String testUuid) {
        try {
    ...
    public void openYandex() {
        WebDriver webDriver;
        webDriver = getWebDriver("");
        webDriver.get(baseUrlYandex);
    }

    synchronized void closeDrivers() {
